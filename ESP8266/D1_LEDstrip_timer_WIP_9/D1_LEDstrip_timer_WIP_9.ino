...
Adafruit_MQTT_Publish tv_sense = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/tvsense");
...
if (! tv_sense.publish(tv_on)) {
    Serial.println(F("tvsense publish Failed"));
  } else {
    Serial.println(F("tvsense publish OK!"));
  }
...
uint16_t time_left = atoi((char *)min_left.lastread);  // convert to a number
