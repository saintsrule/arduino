...
// Setup a feed called 'colour' for subscribing to LED colour
Adafruit_MQTT_Subscribe colour = Adafruit_MQTT_Subscribe(&mqtt, "colour",MQTT_QOS_1);

// Setup a feed called 'flash' for subscribing to LED flash
Adafruit_MQTT_Subscribe flash = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "flash", MQTT_QOS_1);
...
//    Serial.begin(115200);           // Computer Communication
 ...
 // SETUP MQTT CALLBACKS
  colour.setCallback(colourcallback);
//  flash.setCallback(flashcallback);
//  mqtt.subscribe(&colour);
//  mqtt.subscribe(&flash);
...
String colour_MQTT;
 ...
 // mqtt.processPackets(1000); // wait for a second to pick up MQTT messages
...
//  aval=analogRead(ANALOG_PIN);
  aval=Thermister(analogRead(ANALOG_PIN));
//  if (! photocell.publish(x++)) {
if (! photocell.publish(aval)) {
...
//    s += "Analog Pin = ";
    s += String(Thermister(analogRead(ANALOG_PIN)));
//    Serial.print(s);
//    ReadButton();
...
//  double V_NTC = (double)val / 1024;    // think this is for a 1v input max
  double V_NTC = Vcc*(double)val / 1024;
...
void MQTT_connect() {
  ...
  Serial.println("MQTT Connected!");
      ...
      void flashcallback(char *data, uint16_t len) {
        Serial.print("Hey we're in a flash callback, the button value is: ");
        Serial.println(data);
      }
      
      void colourcallback(char *data, uint16_t len) {
        Serial.print("Hey we're in a colour callback, the button value is: ");
  digitalWrite(D10, HIGH);  // Turn the LED off by making the voltage HIGH
        Serial.println(data);
      }
